#!/usr/bin/env python3
"""Apply explicitly reviewed medium-confidence semantic corrections."""
...
EXCLUDED_STATUSES = {"review_suspicious_delta_transfer"}
...
def selected_records(
    ...
    if approval.get("confidence") != "medium":
        raise AuditError("The decisions do not select medium-confidence records.")
    if set(approval.get("statuses", [])) != APPROVED_STATUSES:
        raise AuditError("The approved medium-confidence statuses changed.")
    if set(approval.get("excludedStatuses", [])) != EXCLUDED_STATUSES:
        raise AuditError("The suspicious-transfer exclusion is missing or changed.")
    records = [
        record
    ...
    if approval.get("candidateCount") != len(records):
        raise AuditError("The reviewed medium-confidence candidate count no longer matches.")
    return records
...
def plan_changes(
    ...
    if decisions.get("schemaVersion") != 1:
        raise AuditError("Unsupported medium-confidence review-decision schema.")
    ...
    for record in records:
        ...
        default_text = record.get("proposedText")
        if not isinstance(default_text, str):
            default_text = record.get("finalText")
        reviewed_text = override["text"] if override else default_text
        if not isinstance(reviewed_text, str):
            raise AuditError(f"Record has no reviewed result text: {record.get('epochId')}")
        ...
        changes.append(
            {
                ...
                "action": "apply_reviewed_medium_confidence_and_mark_manual",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Apply reviewed medium-confidence Six Hero corrections."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "migration-reports/six-hero-medium-confidence-review-decisions.json"
        ),
    )
    parser.add_argument("--apply", action="store_true")
    parser.add_argument("--approve-reviewed-medium-confidence", action="store_true")
    return parser
...
def main() -> int:
    ...
    try:
        ...
        if args.apply and not args.approve_reviewed_medium_confidence:
            raise AuditError(
                "Pass --approve-reviewed-medium-confidence with --apply after reviewing the pinned decisions."
            )
        if args.apply:
        ...
        print(
            ...
            "reviewed medium-confidence corrections."
        )
        print(
            json.dumps(
                {
        ...
        if not args.apply:
            print(
                "Dry run only; pass --apply --approve-reviewed-medium-confidence "
